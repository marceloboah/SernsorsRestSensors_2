...
getLastHour:function(callback){
    return db.query("SELECT `id`, `dht_humidity`, `dht_temperature`, `sct_current`, `sct_powerrate`, DATE_FORMAT(dt_created -hour(1),'%d-%m-%Y %k:%i') as dt_created from tb_sensors WHERE dt_created > (now()-INTERVAL 1 HOUR)  ORDER BY id ",callback);
},
